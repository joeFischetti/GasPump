...
public class MainWindow extends JFrame{
	...
	private static String LICENSESALT = "ju4qaew9gjxup2wj";
	...
	public MainWindow() {
		...
		//Check the current license key that's stored in the database
		//
		if(!getLicense()){
	...
	private boolean getLicense(){
		...
		//try-catch block for connecting to sqlite db and pulling the license key
		//
		try {
			Class.forName("org.sqlite.JDBC");
			c = DriverManager.getConnection("jdbc:sqlite:BoatClub");
			...
			stmt = c.createStatement();
			ResultSet rs = stmt.executeQuery("SELECT licenseKey FROM licensing;");


			while(rs.next()){
				licenseKey = new String(rs.getString("licenseKey"));
		...
		System.out.println("The license key is" + licenseKey);
	...
	private boolean newLicense(){
		...
		System.err.println("Prompting for new license, checking validity, and entering into the database");

		String licenseKey = promptForLicenseKey().toUpperCase();


		//Check if the license key entered is valid, and if it is, input it into the database
		//
		if(checkLicense(licenseKey)){
			...
			try {
				...
				stmt = c.createStatement();
				stmt.executeUpdate("CREATE TABLE IF NOT EXISTS \"licensing\" ( 'licenseKey' TEXT NOT NULL PRIMARY KEY);");
				stmt.executeUpdate("INSERT INTO licensing (licenseKey) VALUES ('" + licenseKey + "');");
	...
	private boolean checkLicense(String licenseKeyProvided){
		...
		Calendar cal = Calendar.getInstance(TimeZone.getTimeZone("Eastern/New_York"));
		Date time;
		...
		try{
			...
			System.err.println(e.getClass().getName() + ": " + e.getMessage() );
			System.out.println("Using current system time for year");
			time = new Date();

			
		}

		//Now that we have the current year (either system or NTP), we'll set that as the string
		//
		cal.setTime(time);
		System.out.println("Current date from NTP:  " + cal.get(Calendar.YEAR));
		currentYear = Integer.toString(cal.get(Calendar.YEAR));

		//Well append and prepend the current year to our salt string
		//
		String saltedYear = new String(currentYear + LICENSESALT + currentYear);


		//Print out what we have for debugging
		//
		System.out.println(saltedYear);
	

		try{
			//Compute the has value that a valid license key should have
			//
			MessageDigest md = MessageDigest.getInstance("MD5");
			md.update(saltedYear.getBytes());
			byte[] digest = md.digest();
			String validHash = DatatypeConverter.printHexBinary(digest).toUpperCase();

			//System.out.println("Valid DEBUG ONLY:" + validHash);	
			//
			System.out.println("License key provided:  " + licenseKeyProvided);
			
			if(licenseKeyProvided.equals(validHash)){
				System.out.println("The key provided is valid");
				return true;
			}
			else{
	...
	//
	private String promptForLicenseKey(){
		JPanel popup = new JPanel();
		JLabel prompt = new JLabel("Please enter a valid license key");
		JTextField licenseField = new JTextField(25);

		popup.add(prompt);
		popup.add(licenseField);

		String[] options = new String[]{"Ok", "Cancel"};

		int option = JOptionPane.showInternalOptionDialog(getContentPane(), popup, "Enter license key",
											JOptionPane.NO_OPTION, JOptionPane.PLAIN_MESSAGE,
											null, options, options[0]);

		String licenseKey = new String(licenseField.getText());
