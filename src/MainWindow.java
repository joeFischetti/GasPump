...
public class MainWindow extends JFrame{
	...
	private ClubMember loadFullMember(ClubMember input){
		...
		try {
			...
			ResultSet rs2 = stmt2.executeQuery("SELECT status FROM additional_status, status_codes WHERE member_id='" + member.getID() + "' AND additional_status.status_code = status_codes.code;");

			while(rs2.next()){
				member.addStatus(rs2.getString("status"));
			}
	...
	private void updatePumpDisplay(){
		...
		pumpPanel.setMemberStatus(pumpPanel.getSelectedMember().getStatusList());
